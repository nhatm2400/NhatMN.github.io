...
In addition to the search function, we need two more functions to complete the virtual assistant's features. Follow similar creation steps as the `ragsearch` function, only changing the configuration parameters.

#### 1. Contract Generation Function (generate_contract)

*   **Create Function**:
    *   Name: `generate_contract`
    *   Runtime: `Python 3.12`
![alt text](/images/5-Workshop/5.3-Infrastructure/3.18.fix.png)
*   **Code**: Copy from the `generate_contract.py` file in the source code -> **Deploy**.
```python
import json
import os
import uuid
...
# Lambda RAG-search (deployed in stage 2.3)
...
def load_template_raw_text(metadata: Dict[str, Any]) -> str:
    """
    MVP: Try reading template file content from S3 as text (many .doc/.docx files won't be readable,
    but that's fine, used only for structure reference if decode works).
    Later if you want standard DOCX processing, add python-docx.
    """
...
def build_rag_query(template_metadata: Dict[str, Any], contract_info: Dict[str, Any]) -> str:
    """
    Build query sent to RAG-search:
    - State contract type (template_type, title).
    - Attach contract_info JSON.
    Goal: find legal articles related to contract type and main content.
    """
...
def call_rag_lambda(query: str, language: str = "vi") -> Dict[str, Any]:
    """
    Call Lambda rag_search directly (invokeFunction).
    Lambda rag_search currently returns format:
    ...
    payload = {
        ...
        "filters": {
            "source_type": ["legal"]
            # Can add "field": ["Xây dựng - Đô thị"] if focusing on Real Estate
        }
    }
    ...
    # Case where rag_search is still using make_response(statusCode, body)
    ...
    # If later rag_search is modified to return raw dict, fall here
...
def build_legal_context_text(rag_result: Dict[str, Any]) -> str:
    """
    Receive result from RAG (rag_search) and build into a text block to stuff into prompt.
    """
...
def retrieve_legal_context_for_template(
    ...
    """
    Master function: build query -> call RAG -> build text.
    If error or no result, return "" to avoid blocking main flow.
    """
    try:
...
def build_user_prompt(
    ...
    """
    Prompt for model: provide
    - Template info (type, title, ...).
    - contract_info (JSON).
    - Legal context from RAG.
    - (Optional) snippet template_raw_text if readable.
    """
    title = template_metadata.get("title") or ""
    template_type = template_metadata.get("template_type") or ""
    ...
    # snippet from original template (if any)
...
def call_bedrock_generate_contract(system_prompt: str, user_prompt: str) -> str:
    """
    Call Bedrock (Claude Haiku) to generate contract, return plain text.
    """
...
def to_html_from_text(contract_text: str) -> str:
    """
    Simple: each line -> <p>, empty line -> <br>.
    """
...
def save_generated_to_s3(contract_text: str, contract_html: str) -> Dict[str, str]:
    """
    Save contract_text and contract_html to S3, return paths.
    """
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # 2. (Optional) load raw text of template file from S3
        template_raw_text = load_template_raw_text(metadata)

        # 3. Get legal context from RAG
        ...
        # 5. Call Bedrock to generate contract
...
*   **Permissions**: Add `AmazonBedrockFullAccess` and `AmazonS3FullAccess`.
![alt text](/images/5-Workshop/5.3-Infrastructure/3.22.png)

#### 2. General Chat Function (CallLLM)

*   **Create Function**:
    *   Name: `CallLLM`
    *   Runtime: `Python 3.12`
![alt text](/images/5-Workshop/5.3-Infrastructure/3.19.fix.png)
*   **Code**: Copy from the `CallLLM.py` file in the source code -> **Deploy**.
```python
import json
import os
import base64
...
# ENV: RAG-search Lambda name, e.g., 'rag_search'
...
class ContractInput:
    ...
    # context RAG sent by BE (or None)
...
def parse_event_body(event: Dict[str, Any]) -> Dict[str, Any]:
    """
    Support both calls via API Gateway HTTP API (body is string)
    and direct invoke (event is dict).
    """
    if "body" not in event:
        # direct invoke
        return event
...
# ----------------------------------------------------------------------------- 
# 5. RAG hook: use Lambda invoke instead of API Gateway
# ----------------------------------------------------------------------------- 

def retrieve_legal_context(contract_text: str, language: str) -> str:
    """
    Call Lambda rag_search directly (invokeFunction), return context text to stuff into LLM prompt.
    If RAG fails or not configured, return empty string to avoid blocking main flow.

    Requires:
    - ENV: RAG_FUNCTION_NAME = name of rag_search Lambda
    - IAM: lambda:InvokeFunction on that function
    """
    if not RAG_FUNCTION_NAME:
        return ""

    # Payload sent to rag_search
    payload = {
        "query": contract_text,
        "language": language,
        "top_k": 8,
    ...
    # Case where rag_search returns API format (statusCode + body)
    if isinstance(resp_payload, dict) and "statusCode" in resp_payload:
        status = resp_payload.get("statusCode", 500)
        if status != 200:
        ...
        # If later rag_search returns raw dict, use it directly
        if isinstance(resp_payload, dict):
            result = resp_payload
        else:
            return ""
...
def build_user_prompt_text(contract_text: str, context: str | None = None) -> str:
    """
    Prompt used for TEXT mode.
    Adds context (RAG) if not empty.
    """
    base = (
...
def call_bedrock_text(contract_text: str, language: str) -> str:
    """
    Call Bedrock Converse API with TEXT and return raw text from model.
    Has hook for RAG context.
    """
...
def call_bedrock_document(
    ...
    """
    Call Bedrock Converse API with DOCUMENT (pdf/docx/...) and return raw text from model.

    file_format: pdf, doc, docx, txt, md, html
    context: RAG string (law, template) built by BE (context_rag)
    """
    if not file_name:
        file_name = f"uploaded_contract.{file_format}"

    # Stuff RAG context (if any) into user_text
    user_text = (
...
def parse_model_json(model_output_text: str) -> Tuple[Dict[str, Any], Optional[str]]:
    """
    Robust version: attempts to extract the first JSON object in the output.
    """
    try:
...
def analyze_contract(contract_input: ContractInput) -> Tuple[Dict[str, Any], Optional[str]]:
    """
    Core logic:
    - Select TEXT or DOCUMENT mode.
    - Call corresponding Bedrock.
    ...
    if contract_input.has_file:
        model_output_text = call_bedrock_document(
            ...
            context=contract_input.rag_context,  # RAG context passed by BE (if any)
        )
...
*   **Environment variables**:
    *   `MODEL_ID`: `anthropic.claude-3-haiku-20240307-v1:0`
    *   `RAG_FUNCTION_NAME`: `ragsearch`
    *   `S3_BUCKET_RAW`: `<Your-S3-Bucket-Name>`
